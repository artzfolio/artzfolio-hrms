// ArtzFolio HRMS — Service Worker · cache key artzfolio-hrms-v369-2026-07 (v369: 11 settings-field tooltips —
// passwords, pay-day, factory GPS/geofence radius, auto-clockout buffer, auto-mark-present + late-approval
// thresholds). Full version history in the prior sw_vNNN.js archives. This is a REAL same-origin SW file (a
// separate static asset, NOT a split of the single-file app); the app registers it via
// navigator.serviceWorker.register('sw.js', {scope:'./'}). The CACHE key MUST stay in lockstep with the inline
// fallback CACHE constant in the app HTML. Shell = network-first (fresh deploys land immediately); face-api
// weights = cache-first (offline kiosk); CDN libs = network-first.
const CACHE = 'artzfolio-hrms-v369-2026-07';
