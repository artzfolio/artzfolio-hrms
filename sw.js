// ArtzFolio HRMS — Service Worker · cache key artzfolio-hrms-v532-2026-08-09
// v532 (2026-08-09): cache key bumped v531->v532 — marker-only. GAS: CompOff promoted from the generic
//   backup-blob mirror to a real structured Supabase table with its own per-employee RLS policy (3rd
//   table, after Leaves/Attendance) — _V281_BACKUP_TABLES 46->45. HTML: "46 tables" -> "45 tables" (2
//   spots), status card gained a CompOff line. No SW logic change.
// v531 (2026-08-09): cache key bumped v530->v531 — marker-only. HTML: removed the one-time v525 "Run Full
//   RLS Test" button (its checks were already proven via direct database verification, button never used)
//   and replaced it with a static status card; added a golden-master payroll-parity PASS status line. No SW logic change.
// v530 (2026-08-08): cache key bumped v529->v530 — marker-only, live-audit-found stale "40 tables" text fixed to 46 in Settings HTML. No SW change.
// v529 (2026-08-08): cache key bumped v528->v529 — marker-only, 'PulseQuestions'+'PulseResponses' added to GAS backup mirror. No SW/HTML logic change.
// v528 (2026-08-08): cache key bumped v527->v528 — marker-only, 'Assets' added to GAS backup mirror. No SW/HTML logic change.
// v527 (2026-08-08): cache key bumped v526->v527 — marker-only, 'Candidates'+'OfferLetters' added to GAS backup mirror. No SW/HTML logic change.
// v526 (2026-08-08): cache key bumped v525->v526 — marker-only, 'Tickets' added to GAS backup mirror. No SW/HTML logic change.
// v525 (2026-08-08): cache key bumped v524->v525 — marker-only, new owner-only RLS pilot self-test button in HTML/GAS. No SW logic change.
...
const CACHE = 'artzfolio-hrms-v583-2026-08-14' // v581: SalaryLadder promoted (schema-reconciled, real key, OWNER_GATED, no client fast-read) -- see decision_log/2026.08.14_HRMS_v581_salaryladder-migration_v1.md // v580_prior_note: HTML-ONLY marker bump -- fixed stale "34 tables" Full-backup-mirror Settings text (true count 29) + stale promoted-table enumeration, found live against production during a P0 check. No SW/GAS logic change. See decision_log/2026.08.14_HRMS_v580_backupcount-textfix_v1.md // v579_prior_note: Leave_Balance + Emp_Change_History + Emp_Bonus_Eligibility fast-reads (Batch 6 final sweep), Emp_Change_History hardened to OWNER_GATED in hrms-read v33, Bonus_Ledger/CompensationConfig stale-backup-array bug fixed -- see decision_log/2026.08.14_HRMS_v579_leavebal-changehist-bonuselig-structured_v1.md // v578_prior_note: LoyaltyWallet Monthly Ledger + LoyaltyBonusLedger History owner-only fast-read wrappers (speed pass, no Edge Function change needed -- both tables already OWNER_GATED since v25/v26) -- see decision_log/2026.08.14_HRMS_v578_loyalty-ledger-fastreads_v1.md // v577: PettyCash + OT_Claims promoted to real structured Supabase tables (OWNER_GATED, hrms-read v31) + OT_Claims owner-only fast-read wrapper -- see decision_log/2026.08.14_HRMS_v577_pettycash-otclaims-structured-mirror_v1.md // v576: KRA scoring engine + manager-scoping + Roster stuck-spinner fix -- see decision_log/2026.08.13_HRMS_v576_kra-scoring-mgrscoping-roster_v1.md // v575: Manager/Supervisor KRA default access locked down (kra:true -> kra:false in both client + server role-permission defaults) -- see decision_log/2026.08.13_HRMS_v575_kra-manager-default-lockdown_v1.md // v574: KRA schema fix (fixed 5-slot model + ESS/module-gate unlock), PT_Slabs schema fix, ESS-token audit-log redaction fix, OnboardingChecklist promoted to Supabase (Batch 5 tier 2), _v96Load1on1Notes hardened; // v573: marker-only bump for 4-way lockstep (Batch 5 tier 1: POSH_ICC + OneOnOneNotes promoted to structured Supabase tables + owner-gated fast-read, plus 2 pre-existing OneOnOneNotes bugs fixed -- write payload field-name mismatch that made every note-add fail, and a missing empId filter that showed every employee's notes at once; SW file content itself unchanged).
