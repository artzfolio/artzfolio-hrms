...
const CACHE = 'artzfolio-hrms-v440-2026-07-28'; // v436_advance-modal-remaining-display-fix (2026-07-26): HTML-ONLY FIX: the on-behalf Advance modal's 'Check eligibility' badge read r.canRequest, a field checkAdvanceEligibility has never returned (it returns .remaining) -- so the 'can request now: RsX' clause silently never rendered and the badge showed only the raw ceiling (maxAdvance), not what was actually left after this month's already-taken advances. Backend math was already correct (confirmed against Raj Shakya AZ_EMP_0037's real July payslip: Gross Rs.8,458 x 40% = Rs.3,383 ceiling, minus Rs.2,600 already taken = Rs.783 actually remaining) -- this only fixes the display to show the number the owner expected to see. No GAS/engine change; version bump for lockstep only.
