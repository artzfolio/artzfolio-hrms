...
const CACHE = 'artzfolio-hrms-v420-2026-07-20'; // v420 (2026-07-20): version bump for HTML/GAS/SW lockstep — two owner-requested corrections: (1) the Monthly Late-Block Threshold field in Settings was relabelled with clearer helper text (same Config key, same wiring — the live value was found to be 3, not the documented default 10); (2) the v419 standalone shared-PIN "Kiosk" login was removed entirely (rejected design) and replaced with a normal Role Access Permission ('quickOnboard') that a Manager/Supervisor's existing password login can be granted, surfacing the same Quick Add Employee + Photo Scanning screens via a normal nav item instead of a separate PIN screen. No SW/caching-strategy change, no engine change.
