...
const CACHE = 'artzfolio-hrms-v446-2026-07-29'; // v444_perfreview-hadbase-fix_pdf-note-template-fix_savesalaries-tp-persist_txn-lock-align -- version bump for lockstep only. See HTML/GAS HRMS_VERSION changelog for the 4 fixes this release. // v443_perfreview-edit-fix_daybyday-tp-column_pdf-overflow-fix -- version bump for lockstep only, no service-worker logic changed. See HTML/GAS HRMS_VERSION changelog for the 3 owner-reported bug fixes this release. // v442_timeadjustment-separate-buckets (2026-07-28): SALARY-CRITICAL REDESIGN: Time Adjustment (penalty/accretion) is no longer netted into a day's natural Overtime/Under-time worked-minutes -- owner correction: an accretion silently reducing under-time (or a penalty silently reducing overtime) confuses employees at payslip time. Natural OT/UT are now computed exactly as if Time Adjustment did not exist; Time Adjustment penalty/accretion are priced independently (same per-minute basis as a normal day) and shown as their OWN separate earning/deduction line ("Time Adjustment (Penalty)" / "Time Adjustment (Accretion)") on every payslip surface: admin modal, both PDF renderers, Excel/CSV (3 new columns), and F&F (both early-exit and normal-exit paths). Version bump for lockstep only -- no service-worker logic changed.
