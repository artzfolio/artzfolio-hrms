// ArtzFolio HRMS — Service Worker (v332 — OFFLINE KIOSK: cache-first face-api model weights + prefetch weight manifests on install so kiosk scanning works with ZERO network; cache key -> v332. Was v296 — cache key bump for the defer-CDN build; was v295 — shell cache-first for instant loads; was v294 — FIX: v251 file was truncated/missing the final closer so 'ServiceWorker script evaluation failed' and it never registered on any device; restored + cache key bumped to v294)
...
const CACHE = 'artzfolio-hrms-v332-2026-07'; // v195: bumped so every device drops the stale shell and pulls the face-liveness + Biometric Settings build (in lockstep with the inline CACHE in the app HTML)
const NETWORK_FIRST_HOSTS = ['cdn.jsdelivr.net', 'cdnjs.cloudflare.com', 'unpkg.com'];
// v332 OFFLINE KIOSK: face-api model weights are large, static and version-pinned. Serve them CACHE-FIRST
...
function _v332IsFaceWeight(url){ return FACE_WEIGHT_MARKERS.some(function(m){ return url.indexOf(m) !== -1; }); }

self.addEventListener('install', e => {
  self.skipWaiting();
  // v332: best-effort prefetch the face-api weight MANIFESTS so the first offline boot already has them.
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(caches.open(CACHE).then(async cache => {
    ...
    // v332 OFFLINE KIOSK: face-api model weights are CACHE-FIRST — once cached, serve from cache immediately
    // (and refresh in the background). This makes on-device face matching work with the network fully down.
    if (_v332IsFaceWeight(url)) return cached || fetchPromise;
